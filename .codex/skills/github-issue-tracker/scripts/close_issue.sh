...
if [ -z "$ISSUE_NUMBER" ]; then
  echo "Usage: $0 [--repo <owner/repo|url>] [--ref <sha|branch>] [--file <path[:line]>] [--rel-issue <#num|owner/repo#num>] <issue_number> [\"Closing comment\"]" >&2
  exit 1
fi
...
if [ -n "$LINK_SECTION" ]; then
  if [ -n "$FINAL_COMMENT" ]; then
    ...
    FINAL_COMMENT="$LINK_SECTION"
  fi
fi

if [ -n "$FINAL_COMMENT" ]; then
  gh issue comment "$ISSUE_NUMBER" --repo "$REPO_NAME" --body "$(printf "%b" "$FINAL_COMMENT")"
fi
