...
if [ -z "$TITLE" ] || [ -z "$BODY" ]; then
  echo "Usage: $0 [--repo <owner/repo|url>] [--ref <sha|branch>] [--file <path[:line]>] [--rel-issue <#num|owner/repo#num>] \"Title\" \"Body\"" >&2
  exit 1
fi
...
ISSUE_URL="$(gh issue create --repo "$REPO_NAME" --title "$TITLE" --body "$BODY$LINK_SECTION" | grep -o 'https://.*')"
