...
- **手写预览转换中状态提示**：手写笔画尚未转换完成时，列表内不再显示"图片/手写评论"占位或加载失败提示，改为明确的"手写内容正在转换，完成后将自动刷新"状态，并在原生侧自动定时重试推送快照，转换完成后自动刷新并提示。

## 0.1.16（2026-08-03）

### 新增

- **手写评论预览**：手写评论（含合并了摘录图片的手写评论）现在会在列表中直接渲染预览图，无需再打开原生界面才能看到笔画内容；解析失败时静默回退，不影响其他评论正常显示。

## 0.1.15（2026-08-02）

### 新增

- **链接评论支持一键更新**：卡片链接、摘要链接评论新增「更新链接」按钮（↻），可直接用剪贴板中的 MarginNote 卡片链接替换目标；若原链接存在反向链接，会自动迁移到新目标卡片，不再需要手动删旧建新。
- **Markdown 正文链接可直接定位**：Markdown 评论正文里的 MarginNote 卡片链接现在可以直接点击，跳转到脑图定位对应卡片，无需先把链接单独转成链接评论。

### 优化

- **定位与撤销机制优化**：卡片定位改为通过窗口的 StudyController 执行，撤销分组默认使用不触发全局数据库刷新的模式，减少批量或单卡操作后的界面卡顿。
- **精简操作文案**：统一「转为非摘录模式」为「转为非摘录版」；单卡「单选处理」菜单文案精简为「去掉链接评论」「清空评论」「清空标题」。

## 0.1.14（2026-07-30）

### 新增

- **面板支持原生摘录操作**：评论管理器列表顶部现显示卡片的原生摘录（文本/图片），可独立选择，并与评论一起参与移动、删除、合并、提取为子卡片、复制文本/图片等所有操作；若操作涉及移动/删除/合并摘录，会先自动完成「转为非摘录模式」再继续，全程在同一撤销组内。
- **转为非摘录模式**：新增独立的「转为非摘录模式」操作——批量多选菜单新增入口，单卡「单选处理」菜单同样新增；将有摘录的卡片转换为纯评论卡片，保留评论内容、卡片链接和反向链接。
- **Markdown 与公式渲染**：Markdown 评论现支持常规 Markdown，以及 `$...$`、`$$...$$`、`\\(...\\)`、`\\[...\\]` 数学公式；原有 Markdown 链接定位与编辑入口保持不变。
- **单卡动态评论入口**：单击卡片时可显示原生”评”按钮；点按打开并聚焦评论管理器，长按打开包含已确认六项操作的”单选处理”菜单。
- **卡片操作按钮设置**：评论管理器新增”设置”，可独立控制多选”评论”按钮与单卡”评”按钮；两项默认开启并保存到原生偏好设置。
- **动态布局避让**：单卡”评”按钮会避让 MNPinner、MNLiterature、MNToolbar 与 MN Chat AI 的已显示动态控件，并在弹出菜单稳定后再次计算位置。

### 修复

- **提取合并摘录保留定位**：提取子卡片改为直接挂载当前卡片的克隆并删除反选评论，不再把克隆二次合并到空白卡片；普通合并摘录的来源卡 ID、图片定位数据和其他原始内容会随克隆保留。
- **Mac WebView 空白**：发布构建改用 Safari 11 兼容目标，并在构建阶段拒绝已知不兼容语法，避免 Markdown 依赖使旧版内嵌 WebView 在 React 挂载前解析失败。
- **通用安全 HTML 渲染**：不再识别 MNKB 私有的 `level1` 等类型或转换为插件 class；Markdown 中的通用 HTML 经 DOMPurify 净化后渲染，可保留受控的内联样式，同时移除 `id`、事件属性、危险 URL 与越界布局样式。
- **运行时缺失时仍清理界面**：即使断开场景时 MN Utils 已不可用，也会移除评论面板和单卡浮动按钮，避免残留失效控件。
- **合并摘录术语修正**：评论管理器将 `mergedImageComment` / `mergedImageCommentWithDrawing` 显示为”合并摘录”/”合并摘录+手写”，避免把可定位文档的合并摘录误称为纯图片。
- **单选菜单点击无反应**：按 MarginNote `MenuController` 的真实 selector 契约直接接收菜单项 `param`，修复所有单选处理项误读 `sender.param` 后静默失败的问题；失败时现在会显示 HUD 并写入日志。
- **Dynamic 单卡动作误判未选卡片**：修复”转为非摘录模式”漏传 `allowSingle`、错误套用多选卡片门槛的问题；单卡动作现在始终按菜单冻结的卡片 ID 执行，不再依赖动作发生时的当前选区。
- **Dynamic 单卡操作去除二次确认**：单选菜单已经明确绑定一张卡片，六项操作现在点击后直接执行；多选批处理仍保留影响统计和确认步骤。
- **Dynamic 多窗口与模式切换隔离**：仅当前窗口响应卡片 PopupMenu 通知；单选和多选入口切换时会主动清理另一模式的浮动按钮与上下文。
- **Dynamic 生命周期清理**：插件实例断开时移除原生”评”按钮，重载时也会清理同 tag 的旧实例按钮，避免按钮继续向已失效的 addon selector 发送事件。
...
- **批量去掉所有链接**：批处理菜单新增「去掉所有链接」操作，一次性移除多选卡片中的纯卡片链接评论（`linkComment`/`summaryComment`），操作前会提示受影响卡片数和链接条数；仅删除卡片自身的链接评论，不处理正文中的 Markdown 行内链接，也不清理目标卡片的反向链接

### 优化

- **运行时依赖隔离**：`main.js` 加载 `vendor/mnutils`、`vendor/mnnote` 前先检测宿主环境是否已提供 `MNUtil`/`MNNote`，避免与其他已加载 MNUtils 的插件重复注册产生冲突
- **区域检测报错静默**：修复内置 MNUtils 中 IP 归属地检测在遇到 `ipapi.co` 反爬虫 HTML 响应时，会把整段 HTML 当作连接错误提示给用户的问题，现在此类响应仅做静默降级处理
...
- **行内合并对话框 — 链接材料分层显示**：链接类型材料（卡片链接、总结链接）现在显示为「标题 + 链接」两行结构，标题字号 13px 加粗，链接字号 12px 次要色；优先使用卡片标题（`linkedNoteTitle`），标题缺失时回退到链接 URL，避免在材料列表中看到大段重复的 URL
- **提取评论对话框增强**：标题输入框旁新增「清空标题」按钮，快速清空预填充文本
- **卡片标题设置容错**：`setNoteTitle` 函数增强，尝试多个属性路径（`title`、`noteTitle`、`note.noteTitle`），提升跨版本兼容性
